...
class VocabAgent(AgentBase):
    ...
    支持会话配置（难度级别）。
    """
    def __init__(self, session_id=None, config: Optional[ConversationConfig] = None):
    ...
    def restart_session(self, session_id=None, config: Optional[ConversationConfig] = None):
        """
        重新启动会话，清除会话历史。
