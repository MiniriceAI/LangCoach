...
- **✨ 自定义场景** (Custom Scenario) - 输入任意场景描述，AI 自动分析并生成对话环境（新增）
...
│  ┌─────────────────────────────────────────────────────┐   │
│  │  /api/chat/*             - 对话管理                  │   │
│  │  /api/custom-scenario/*  - 自定义场景 (新增)         │   │
│  │  /api/transcribe         - 语音转文字 (Whisper)      │   │
│  │  /api/synthesize         - 文字转语音 (Edge-TTS)     │   │
│  │  /api/dictionary         - 词典查询                  │   │
│  │  /api/auth/*             - 用户认证                  │   │
│  └─────────────────────────────────────────────────────┘   │
│                            │                                │
...
│  ┌─────────────────────────────────────────────────────┐   │
│  │              LLM Agent Layer                         │   │
│  │  ScenarioAgent (job_interview, hotel_checkin, etc)  │   │
│  │  CustomScenario (动态生成的自定义场景)               │   │
│  └─────────────────────────────────────────────────────┘   │
│                            │                                │
│                            ▼                                │
│  ┌─────────────────────────────────────────────────────┐   │
│  │    LLM Factory (Ollama/DeepSeek/OpenAI)             │   │
│  └─────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────┘
```

> 📖 详细的小程序 API 文档请参考 [MINIPROGRAM_API.md](MINIPROGRAM_API.md)
...
#### 6. 启动小程序 API 服务 / Start Mini Program API (可选)
...
小程序 API 服务运行在 `http://localhost:8600`，提供以下功能：
- 对话管理 (`/api/chat/*`)
...
│   ├── api/                    # API 模块 (小程序后端)
│   │   ├── miniprogram_api.py  # 小程序统一 API (新增)
│   │   └── speech_api.py       # 语音 API (兼容旧版)
│   ├── tts/                    # 语音合成模块
│   │   └── service.py          # TTS 服务
│   ├── stt/                    # 语音识别模块
│   │   └── service.py          # STT 服务
...
├── run_miniprogram_api.sh      # 小程序 API 启动脚本 (新增)
├── run_speech_api.sh           # 语音 API 启动脚本
...
├── MINIPROGRAM_API.md          # 小程序 API 文档 (新增)
