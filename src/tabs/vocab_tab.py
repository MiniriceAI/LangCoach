...
# 重新启动词汇学习聊天机器人会话
def restart_vocab_study_chatbot(difficulty: str):
    """
    重置词汇学习会话：
    1. 清除会话历史
    ...
    """
    LOG.info(f"[Vocab] 重置会话，开始新的一关，难度={difficulty}")

    # 创建配置
    config = create_config(difficulty=difficulty)

    # 清除会话历史，确保每次重启都是全新的会话
    vocab_agent.restart_session(config=config)
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                difficulty_dropdown = gr.Dropdown(
                    ...
                    label="难度级别",
                    value="medium",
                    info="选择词汇难度：初级(A1/A2)、中级(B1/B2)、高级(C1/C2)",
                )
